...
(define (key n)
  (let ((e (car (cdr (get-nth-quadruple n)))) (f (car (cdr (cdr (get-nth-quadruple n))))))
  (map (lambda (x) (modulo x 27))
        (list (+ (* e e) (* 2 f e))
        (+ (* 2 f f) (* 2 f e))
        (+ (* 2 f f) (* e e) (* 2 f e))
        (- (* e e) (* 2 f e))
...
(define (message->codes message)
  (map (lambda (x) (if (= (char->integer x) 32) 0
                       (- (char->integer x) 96))) (string->list message))
  )
...
(define (codes->message codes)
  (list->string (map (lambda (x) (if (= x 0) (integer->char 32)
                                     (integer->char (+ x 96)))) codes))
  )
...
(define (extend-key key size)
  (cond ((= size (length key)) key)
        ((< size (length key)) (extend-key (cut key) size))
        ((> size (length key)) (extend-key (append key key) size)))
  )
...
(define (encript-decrypt-codes message key f)
  (map (lambda (nr1 nr2) (modulo (f nr1 nr2) 27)) message key)
  )

(define (encrypt-codes message key)
  (encript-decrypt-codes message (extend-key key (length message)) +)
  )
...
(define (encrypt-decrypt-message message key f)
  (codes->message
   (map (lambda (nr1 nr2) (modulo (f nr1 nr2) 27))
       message
       (extend-key key (length message))))
  )

(define (encrypt-message message key)
  (encrypt-decrypt-message (message->codes message) key +)
  )
