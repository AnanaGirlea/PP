...
(define (pairs G H)
  (let recur ([stream1 G]
               ...
               [i 1]
               [j 0])
        (if (<= i j) (recur G (stream-rest stream2) (+ 1 i) 0)
            (stream-cons (cons (stream-first stream1) (stream-first stream2))
                         (recur (stream-rest stream1) stream2 i (+ 1 j)))))
  )
...
(define (listodds n)
  (let fct ([nr n])
    (stream-cons nr (fct (+ 2 nr)))))

(define gh-pairs-stream
  (stream-filter (lambda (nr) (= (gcd (car nr) (cdr nr)) 1)) (pairs (listodds 1) (listodds 3)))
  )
...
(define ppt-stream-in-pair-order
  (stream-map (lambda (nr) (list (* (car nr) (cdr nr))
                                 (/ (- (* (cdr nr) (cdr nr)) (* (car nr) (car nr)))2)
                                 (/ (+ (* (cdr nr) (cdr nr)) (* (car nr) (car nr)))2)))
              gh-pairs-stream)
  )
